...
/* spraak met vrouwenstem */
function speak(text){
    let s = new SpeechSynthesisUtterance(text);
    let voices = speechSynthesis.getVoices();
    let femaleVoice = voices.find(v => v.lang.includes("nl") && v.name.toLowerCase().includes("female"))
...
/* kies letters op basis van level */
function getLettersForLevel(word){
    let letters = word.split("");
    if(level === 1) return letters; // alles correct
    if(level === 2){
        let first = letters[0];
        let rest = letters.slice(1).sort(()=>Math.random()-0.5);
        return [first, ...rest];
    }
    return letters.sort(()=>Math.random()-0.5); // volledig random
}
...
function moveLetters(){
    movingLetters.forEach(el=>{
        ...
        x += (Math.random()-0.5) * 2; // traag bewegen
        y += (Math.random()-0.5) * 2;
        el.style.left = x + "px";
        el.style.top = y + "px";
    });
